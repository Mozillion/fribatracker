...
abstract class Course implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Moz\\Map\\CourseTableMap';
    ...
    /**
     * @var        ObjectCollection|ChildFairway[] Collection to store aggregation of ChildFairway objects.
     */
    protected $collFairways;
    protected $collFairwaysPartial;
     ...
     * @var ObjectCollection|ChildFairway[]
     */
    protected $fairwaysScheduledForDeletion = null;
    ...
    /**
     * Initializes internal state of Moz\Base\Course object.
     */
     ...
     * @return $this|\Moz\Course The current object (for fluent API support)
     */
    ...
    public function setRating($v)
    {
        ...
        if ($this->rating !== $v) {
            $this->rating = $v;
            $this->modifiedColumns[CourseTableMap::COL_RATING] = true;
        }
    ...
    public function setUserId($v)
    {
        ...
        if ($this->user_id !== $v) {
            $this->user_id = $v;
            $this->modifiedColumns[CourseTableMap::COL_USER_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : CourseTableMap::translateFieldName('Rating', TableMap::TYPE_PHPNAME, $indexType)];
            $this->rating = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : CourseTableMap::translateFieldName('UserId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Moz\\Course'), 0, $e);
        }
    }
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->fairwaysScheduledForDeletion !== null) {
                if (!$this->fairwaysScheduledForDeletion->isEmpty()) {
                    \Moz\FairwayQuery::create()
                        ->filterByPrimaryKeys($this->fairwaysScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->fairwaysScheduledForDeletion = null;
                }
            }

            if ($this->collFairways !== null) {
                foreach ($this->collFairways as $referrerFK) {
            ...
            if ($this->layoutsScheduledForDeletion !== null) {
                if (!$this->layoutsScheduledForDeletion->isEmpty()) {
                    \Moz\LayoutQuery::create()
                        ->filterByPrimaryKeys($this->layoutsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(CourseTableMap::COL_RATING)) {
            $modifiedColumns[':p' . $index++]  = 'rating';
        }
        if ($this->isColumnModified(CourseTableMap::COL_USER_ID)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collFairways) {
                ...
                $result[$key] = $this->collFairways->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Moz\Course The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CourseTableMap::COL_RATING)) {
            $criteria->add(CourseTableMap::COL_RATING, $this->rating);
        }
        if ($this->isColumnModified(CourseTableMap::COL_USER_ID)) {
            $criteria->add(CourseTableMap::COL_USER_ID, $this->user_id);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Moz\Course (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getFairways() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addFairway($relObj->copy($deepCopy));
                }
            }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Moz\Course Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if ('Fairway' == $relationName) {
            return $this->initFairways();
        }
    ...
    /**
     * Clears out the collFairways collection
     *
     ...
     * @see        addFairways()
     */
    public function clearFairways()
    {
        $this->collFairways = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collFairways collection loaded partially.
     */
    public function resetPartialFairways($v = true)
    {
        $this->collFairwaysPartial = $v;
    }

    /**
     * Initializes the collFairways collection.
     *
     * By default this just sets the collFairways collection to an empty array (like clearcollFairways());
     ...
     * @return void
     */
    public function initFairways($overrideExisting = true)
    {
        if (null !== $this->collFairways && !$overrideExisting) {
            return;
        }

        $collectionClassName = FairwayTableMap::getTableMap()->getCollectionClassName();

        $this->collFairways = new $collectionClassName;
        $this->collFairways->setModel('\Moz\Fairway');
    }

    /**
     * Gets an array of ChildFairway objects which contain a foreign key that references this object.
     *
     ...
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildFairway[] List of ChildFairway objects
     * @throws PropelException
     */
    public function getFairways(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collFairwaysPartial && !$this->isNew();
        if (null === $this->collFairways || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collFairways) {
                // return empty collection
                $this->initFairways();
            } else {
                $collFairways = ChildFairwayQuery::create(null, $criteria)
                    ->filterByCourse($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collFairwaysPartial && count($collFairways)) {
                        $this->initFairways(false);

                        foreach ($collFairways as $obj) {
                            if (false == $this->collFairways->contains($obj)) {
                                $this->collFairways->append($obj);
                            }
                        }

                        $this->collFairwaysPartial = true;
                    }

                    return $collFairways;
                }

                if ($partial && $this->collFairways) {
                    foreach ($this->collFairways as $obj) {
                        if ($obj->isNew()) {
                            $collFairways[] = $obj;
                        }
                    }
                ...
                $this->collFairways = $collFairways;
                $this->collFairwaysPartial = false;
            }
        }

        return $this->collFairways;
    }

    /**
     * Sets a collection of ChildFairway objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $fairways A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildCourse The current object (for fluent API support)
     */
    public function setFairways(Collection $fairways, ConnectionInterface $con = null)
    {
        /** @var ChildFairway[] $fairwaysToDelete */
        $fairwaysToDelete = $this->getFairways(new Criteria(), $con)->diff($fairways);

        
        $this->fairwaysScheduledForDeletion = $fairwaysToDelete;

        foreach ($fairwaysToDelete as $fairwayRemoved) {
            $fairwayRemoved->setCourse(null);
        }

        $this->collFairways = null;
        foreach ($fairways as $fairway) {
            $this->addFairway($fairway);
        }

        $this->collFairways = $fairways;
        $this->collFairwaysPartial = false;
    ...
    /**
     * Returns the number of related Fairway objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related Fairway objects.
     * @throws PropelException
     */
    public function countFairways(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collFairwaysPartial && !$this->isNew();
        if (null === $this->collFairways || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collFairways) {
            ...
            if ($partial && !$criteria) {
                return count($this->getFairways());
            }

            $query = ChildFairwayQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collFairways);
    }

    /**
     * Method called to associate a ChildFairway object to this object
     * through the ChildFairway foreign key attribute.
     *
     * @param  ChildFairway $l ChildFairway
     * @return $this|\Moz\Course The current object (for fluent API support)
     */
    public function addFairway(ChildFairway $l)
    {
        if ($this->collFairways === null) {
            $this->initFairways();
            $this->collFairwaysPartial = true;
        ...
        if (!$this->collFairways->contains($l)) {
            $this->doAddFairway($l);

            if ($this->fairwaysScheduledForDeletion and $this->fairwaysScheduledForDeletion->contains($l)) {
                $this->fairwaysScheduledForDeletion->remove($this->fairwaysScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildFairway $fairway The ChildFairway object to add.
     */
    protected function doAddFairway(ChildFairway $fairway)
    {
        $this->collFairways[]= $fairway;
        $fairway->setCourse($this);
    }

    /**
     * @param  ChildFairway $fairway The ChildFairway object to remove.
     * @return $this|ChildCourse The current object (for fluent API support)
     */
    public function removeFairway(ChildFairway $fairway)
    {
        if ($this->getFairways()->contains($fairway)) {
            $pos = $this->collFairways->search($fairway);
            $this->collFairways->remove($pos);
            if (null === $this->fairwaysScheduledForDeletion) {
                $this->fairwaysScheduledForDeletion = clone $this->collFairways;
                $this->fairwaysScheduledForDeletion->clear();
            }
            $this->fairwaysScheduledForDeletion[]= clone $fairway;
            $fairway->setCourse(null);
        }
    ...
    public function removeLayout(ChildLayout $layout)
    {
        if ($this->getLayouts()->contains($layout)) {
            ...
            $this->layoutsScheduledForDeletion[]= clone $layout;
            $layout->setCourse(null);
        }
     ...
     * @return ObjectCollection|ChildLayout[] List of ChildLayout objects
     */
    public function getLayoutsJoinUser(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildLayoutQuery::create(null, $criteria);
    ...
    {
        if ($deep) {
            if ($this->collFairways) {
                foreach ($this->collFairways as $o) {
